...
reading the tree. Written 2026-09-19 against HEAD `50b5044` (version 0.3.7).
...
4. **`Client::result()` is emitted queued and the object is freed by a queued
   `disposeObject` behind it.** Move fields out in the slot; never retain the raw
   `td_api::Object *`. `injectUpdate` follows the same discipline.
...
meegramd (src/daemon/)                       meegram (src/)
  ...
  broadcast every line to every UI   │  unix     │ emit result(Object*) queued
...
```
main.qml (PageStackWindow; theme colours, isSided(), openChat/openProfilePage/openPhoto, save plumbing)
...
- Bubble vs flat layout: `appWindow.isSided(model.isOutgoing)` answers "drawn as
  outgoing"; only the action menu and the unplayed-note dot read `model.isOutgoing`.
- Dark theme: `theme.inverted` from `Settings::invertedTheme`; app colours are
  properties on `appWindow`; theme glyphs use `-inverse` assets via `themeIcon()`; bubble
  PNGs are regenerated by `tools/make_inverted_assets.py`.
...
**Housekeeping**: the four commits after `release 0.3.5` (theming, dark theme, bubble
switch, flat mode) are not in `debian/changelog`, whose top entry is 0.3.7. "Show
bubbles" is the one untranslated string by design. `docs/architecture.md`'s
notification data-flow section describes the transport-OFF path and says so.
